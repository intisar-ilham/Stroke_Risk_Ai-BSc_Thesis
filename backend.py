...
def startup_event():
    global scaler, label_encoders, lime_explainer, rf_model, shap_explainer
    ...
    try:
        dataset_path = os.path.join(base_dir, "healthcare-dataset-stroke-data.csv")
        if not os.path.exists(dataset_path):
            print(f"Warning: Dataset not found at {dataset_path}. LIME needs the training dataset.")
...
def explain_prediction(patient: PatientInput):
    ...
    try:
        ...
        shap_values = shap_explainer.shap_values(df_proc)
        # For classification, shap_values is typically a list for each class or an array (samples, features, classes)
        ...
        for i, col in enumerate(features_cols):
            formatted_shap.append({
                "feature": col,
                "weight": float(sv[i]),
                "influence": "Positive (Increases risk)" if sv[i] > 0 else "Negative (Decreases risk)"
            })
            
        return {
            "status": "success",
            "model_name": "Random Forest",
            "lime_explanations": formatted_explanations,
            "shap_explanations": formatted_shap,
