...
const limeLoading = document.getElementById('lime-loading');
const explanationChartWrapper = document.getElementById('explanation-chart-wrapper');
...
async function loadLimeExplanation(patientData) {
    ...
    try {
        ...
        if (data.status === 'success') {
            ...
            explanationChartWrapper.classList.remove('hidden');
            renderExplanationChart(data.explanations);
        } else {
            throw new Error(data.message || 'LIME explanation calculation failed.');
        }
    } catch (error) {
        console.error("LIME explanation load failed:", error);
        limeLoading.classList.add('hidden');
        explanationChartWrapper.classList.remove('hidden');
        ...
        ctx.fillText("Failed to load LIME explanation.", 200, 150);
    }
}

// Render LIME Explanation Chart
function renderExplanationChart(explanations) {
    ...
    const labels = sortedExplanations.map(e => e.description);
        ...
        options: {
            ...
            scales: {
                x: {
                    ...
                    title: {
                        display: true,
                        text: 'Feature Influence (LIME Weight)',
                        color: '#9ca3af',
                        font: {
                            family: 'Inter',
                            size: 11
